    ...
    return (
        <section style={{ border: '1px solid #DFE0EB', borderRadius: '8px' }} className='bg-white mt-4'>
            <div className="d-flex justify-content-between align-items-center px-3 pt-3">
                <h2 className='ticket-status-title'>Unresolved tickets | <span style={{ color: '#E15E68' }}>{tickets.filter(ticket => ticket.status === "Pending").length}</span></h2>
            ...
            <div className="pb-3">
                <div style={{ borderBottom: '1px solid #DFE0EB' }} className="d-flex justify-content-between align-items-center py-3">
                    ...
                    <div className="ticket-status-content-number pe-3">{vehicleTickets.filter(ticket => ticket.status === "Pending").length}</div>
                </div>

                <div style={{ borderBottom: '1px solid #DFE0EB' }} className="d-flex justify-content-between align-items-center py-3">
                    <div className="ticket-status-content-title ps-3">Picker Management</div>
                    <div className="ticket-status-content-number pe-3">{pickerTickets.filter(ticket => ticket.status === "Pending").length}</div>
                </div>

                <div style={{ borderBottom: '1px solid #DFE0EB' }} className="d-flex justify-content-between align-items-center py-3">
                    <div className="ticket-status-content-title ps-3">Sorter Management</div>
                    <div className="ticket-status-content-number pe-3">{sorterTickets.filter(ticket => ticket.status === "Pending").length}</div>
                </div>
            </div>
        </section>
    );
