...
import ViewSorterListData from './ViewSorterListData';
let valid

const UpdateSorterList = () => {
      ...
      const handleChange = (file) => {
            document.getElementById('sorter-loading-spinner').style.display = 'block'
            setFileUploadError("")
            setFile(file);
            setFileName(file.name)
            setFileSize(bytesToSize(file.size))
            document.getElementById('sorter-uploaded-container').style.display = 'block'
            document.getElementById('sorter-upload-container').style.display = 'none'
            ...
            reader.onload = (e) => {
                  ...
                  if (!valid) {
                        document.getElementById('sorter-loading-spinner').style.display = 'none'
                        setFileUploadError("File Format Mismatch. Please Check again and upload")
                        document.getElementById('sorter-upload-container').style.display = 'block'
                  }
            }
      ...
      const handleUndo = () => {
            setFile(null)
            setData([])
            document.getElementById('sorter-uploaded-container').style.display = 'none'
            document.getElementById('sorter-upload-container').style.display = 'block'
      }
      ...
      return (
            ...
            <h2 className='h6 font-ibm ms-2'>Update Sorter List</h2>
                <div id="sorter-upload-container">
                    <FileUploader
                        children={
                ...
                <div style={{ display: 'none' }} id="sorter-uploaded-container">
                    {
            ...
            {
                data.length > 0 ?
                    <ViewSorterListData sorterData={data} />
                    :
                    <div style={{ display: 'none' }} id='sorter-loading-spinner'>
...
export default UpdateSorterList;